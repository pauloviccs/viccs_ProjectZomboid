...
function VICCS.Zombies.pulseSound(deviceObj, x, y, z, masterVolume, isHeadphones)
    ...
    -- Calcula o raio de atracao proporcional ao volume (0.0 a 1.0) baseado nas configuracoes de Sandbox
    local maxRadius = (VICCS.Config and VICCS.Config.getSandboxVar and VICCS.Config.getSandboxVar("ZombieAttractionRadius", 25)) or 25
    local radius = math.floor(masterVolume * maxRadius)
    local intensity = math.floor(masterVolume * (maxRadius * 3))
